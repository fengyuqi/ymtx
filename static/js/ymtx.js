...
function showConfirm(title, text, fn_ok, fn_cancel) {
    var s = '<div id="div-confirm">' +
            '<a href="#0" class="pure-modal-close pure-close"></a>' +
            '<h1 class="x-title"></h1>' +
            '<p class="x-text"></p>' +
            '<hr>' +
            '<button class="pure-button pure-button-primary x-ok">是</button>' +
            '&nbsp;&nbsp;&nbsp;' +
            '<button class="pure-button">否</button>' +
            '</div>';
    $('body').append(s);
    var m = $('#div-confirm');
    var modal = new $('#div-confirm');
